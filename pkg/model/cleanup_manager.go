...
func (cm *CleanupManager) RemoveSubsetsAndConsumers(ctx context.Context, subsetsAndConsumers SubsetsAndConsumersMap) (inProgress bool, _ error) {
	for name, typ := range subsetsAndConsumers {
		if typ == riskifiedv1alpha1.CONSUMER {
			processing, err := cm.removeConsumer(ctx, name)
			if err != nil {
			...
			processing, err := cm.removeSubset(ctx, name)
			if err != nil {
...
func (cm *CleanupManager) DeleteAllResources(ctx context.Context) (ctrl.Result, error) {
	...
	if slices.Contains(cm.DE.Finalizers, names.DeleteDeployments) {
		count, err := cm.deleteAllDeployments(ctx)
		if err != nil {
			cm.Log.Error(err, "cleanup all resources")
			return ctrl.Result{}, err
		}
		if count == 0 {
			if err := cm.deleteFinalizer(ctx, names.DeleteDeployments); err != nil {
	...
	if slices.Contains(cm.DE.Finalizers, names.DeleteDestinationRules) {
		count, err := cm.deleteAllDestinationRules(ctx)
		if err != nil {
			cm.Log.Error(err, "cleanup all resources")
			return ctrl.Result{}, err
		}
		if count == 0 {
			if err := cm.deleteFinalizer(ctx, names.DeleteDestinationRules); err != nil {
	...
	if slices.Contains(cm.DE.Finalizers, names.CleanupVirtualServices) {
		if err := cm.cleanupVirtualServices(ctx); err != nil {
			cm.Log.Error(err, "cleanup all resources")
			return ctrl.Result{}, err
		}
		if err := cm.deleteFinalizer(ctx, names.CleanupVirtualServices); err != nil {
...
func (cm *CleanupManager) deleteFinalizer(ctx context.Context, finalizer string) error {
	...
	if err := cm.Update(ctx, cm.DE); err != nil {
...
func (cm *CleanupManager) deleteAllDeployments(ctx context.Context) (int, error) {
	...
	for _, item := range deployments {
		found, err := cm.deleteDeployment(ctx, item)
		if err != nil {
...
func (cm *CleanupManager) deleteAllDestinationRules(ctx context.Context) (int, error) {
	...
	for _, item := range drs {
		...
		found, err := cm.deleteDestinationRule(ctx, item)
		if err != nil {
...
func (cm *CleanupManager) cleanupVirtualServices(ctx context.Context) error {
	var vss []riskifiedv1alpha1.ResourceStatus
	for _, s := range cm.DE.Status.SubsetsStatus {
		vss = append(vss, s.VirtualServices...)
	}
	for _, item := range vss {
		if err := cm.cleanupVirtualService(ctx, item); err != nil {
...
func (cm *CleanupManager) cleanupVirtualService(ctx context.Context, vs riskifiedv1alpha1.ResourceStatus) error {
	...
	if err := cm.Get(ctx, types.NamespacedName{Name: vs.Name, Namespace: vs.Namespace}, &found); err != nil {
	...
	if err := cm.Update(ctx, &found); err != nil {
...
func (cm *CleanupManager) removeConsumer(ctx context.Context, name string) (processing bool, err error) {
	st, ok := cm.DE.Status.ConsumersStatus[name]
	if ok {
		found, err := cm.deleteDeployment(ctx, st.ResourceStatus)
		if err != nil {
		...
		if found {
			...
			return true, cm.Status().Update(ctx, cm.DE)
		} else {
			delete(cm.DE.Status.ConsumersStatus, name)
			return false, cm.Status().Update(ctx, cm.DE)
		}
	}
...
func (cm *CleanupManager) removeSubset(ctx context.Context, name string) (processing bool, _ error) {
	st, ok := cm.DE.Status.SubsetsStatus[name]
	exists := false
	if ok {
		found, err := cm.deleteDeployment(ctx, st.Deployment)
		if err != nil {
		...
		for _, dr := range st.DestinationRules {
			found, err := cm.deleteDestinationRule(ctx, dr)
			if err != nil {
				return true, fmt.Errorf("removing subset: %w", err)
			}
			if found {
		...
		for _, vs := range st.VirtualServices {
			if err := cm.cleanupVirtualService(ctx, vs); err != nil {
				return true, fmt.Errorf("removing subset: %w", err)
			}
		}
	}
...
func (cm *CleanupManager) deleteDeployment(ctx context.Context, deployment riskifiedv1alpha1.ResourceStatus) (found bool, _ error) {
	dep := appsv1.Deployment{}
	if err := cm.Get(ctx, types.NamespacedName{Name: deployment.Name, Namespace: deployment.Namespace}, &dep); err != nil {
		if errors.IsNotFound(err) {
			return false, nil
		}
		return false, fmt.Errorf("fetching deployment for deletion: %w", err)
	}
	if err := cm.Delete(ctx, &dep); err != nil {
		return true, fmt.Errorf("deleting deployment: %w", err)
	}
	return true, nil
}

func (cm *CleanupManager) deleteDestinationRule(ctx context.Context, dr riskifiedv1alpha1.ResourceStatus) (found bool, _ error) {
	toDelete := istionetwork.DestinationRule{}
	if err := cm.Get(ctx, types.NamespacedName{Name: dr.Name, Namespace: dr.Namespace}, &toDelete); err != nil {
		if errors.IsNotFound(err) {
			return false, nil
		}
